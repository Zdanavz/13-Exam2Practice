...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        num_append=min(space,len(additional_contents))
        add=''
        for k in range(num_append):
            add=add+additional_contents[k]
        self.contents=self.contents+add

        ret=''
        for k in range(num_append,len(additional_contents)):
    ...
    def double_then_shrink(self, new_volume):
        ...
        return len(clipped1)+len(clipped2)
    ...
    def reset(self):
        ...
        self.contents=self.resetc
        self.volume=self.resetv
